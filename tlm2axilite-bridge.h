...
public:
	...
	{    //entra aquí
		if (aligner_enable) {
            ...
            //realiza los dos b_transport, ¿el proxy inicia una transaccion de proxy_init_socket con proxy_target_socket y el proxy_target_socket activa le b_transport que se encarga de comunicarse con la IP usando los puertos creados?
...
private:
	...
	virtual void b_transport(tlm::tlm_generic_payload& trans,
					sc_time& delay)
	{
        cout << "b_transport del tlm2axiitebridge" << endl;
		...
		if (resetn.read() && Validate(tr)) {
			...
			if (trans.is_read()) {
                    cout << "a" << endl; //entra aqui y lee
				rdTransFifo.write(&tr);
			} else {
                    cout << "b" << endl;
				wrTransFifo.write(&tr); // si entra aquí se comunica con el IP y este IP entra en axi2tlm que se comunica con iconnect  y este con zynq por medio de i_sk[3]
			}
		...
		m_mutex.unlock();
cout << "salgo del b_transport del tlm2axiitebridge" << endl;
	}
